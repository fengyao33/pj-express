...
export const seatSchema = new Schema<ISeat>({
  ...
  status: { type: Number, required: [true, '請輸入座位狀態'], enum: [0, 2, 3] },
},{_id:false});
...
const theaterSchema = new Schema<ITheater>({
  ...
  introduction: { type: String, required: [true, '簡介欄位:introduction 未輸入'] },
  trafficAdvice: { type: [String], required: [true, '交通指南欄位:trafficAdvice 未輸入'] },
  ticketTypeInfoIds: [{ type: Schema.Types.ObjectId, required: [true, '票價資訊欄位:ticketTypeInfoIds 未輸入'], ref: TicketType.collection.name}]
}, {
  versionKey: false,
  collection: 'theaters',
  timestamps: true

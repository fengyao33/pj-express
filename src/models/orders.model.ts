...
const orderSchema = new Schema<IOrder>(
  {
    movieName: {
      type: String,
      required: [true, "請輸入電影名稱欄位:movieName"],
    },
    movieNameEng: {
      type: String,
      required: [true, "請輸入電影英文名稱欄位:movieNameEng"],
    },
    theaterName: {
      type: String,
      required: [true, "請輸入戲院名稱欄位:theaterName"],
    },
    movieDatetime: {
      type: String,
      required: [true, "請輸入電影時刻欄位:movieDatetime"],
    },
    ticketTypeName: {
      type: String,
    ...
    seats: {
      ...
      validate: {
        validator: (seats: string[]) =>
          seats.every((seat) => /\d+排\d+/.test(seat)),

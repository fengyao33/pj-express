...
const seatSchemaForSession = new Schema<ISeat>({
  ...seatSchema.obj,
  status: { type: Number, required: [true, '請輸入座位狀態'], enum: [0, 1, 2, 3] },
},{_id:false});
...
const sessionSchema = new Schema<ISession>({
  ...
  ticketTypeIds: [{ type: Schema.Types.ObjectId, required: [true, '票種資訊id欄位:ticketTypeId 未輸入'], ref: TicketType.collection.name}]
}, {
  versionKey: false,
  collection: 'sessions',
  timestamps: true
